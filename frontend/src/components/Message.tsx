...
function CodeBlock({ code, lang }: { code: string; lang?: string }) {
  ...
  return (
    <div className="my-3 rounded-xl border border-white/10 bg-black/60 overflow-hidden">
      ...
      <pre
        className={[
          "p-3 text-xs md:text-sm leading-relaxed max-h-96 overflow-auto",
...
export default function Message({ role, content, sources }: MessageProps) {
  ...
  const rendered = useMemo(() => {
    ...
    // Apply small, safe normalizations *outside* code fences only
    const healed = normalizeOutsideFences(raw, (segment) => {
  ...
  return (
    <div className={isUser ? "flex justify-end" : "flex justify-start"}>
      <div
        className={[
          "relative max-w-[85%] rounded-2xl p-4 border backdrop-blur-md bg-gradient-to-br",
      ...
      >
        ...
        <div className="prose prose-invert prose-xs sm:prose-sm md:prose-base max-w-none [--tw-prose-pre-code:theme(fontSize.sm)]">
          <ReactMarkdown
            remarkPlugins={[
              remarkGfm,
              remarkBreaks,
            ...
            rehypePlugins={[
              // highlight first, then sanitize with a schema that allows hljs classes
              [rehypeHighlight, { subset: ["bash", "json", "javascript", "typescript", "tsx", "python", "markdown", "css", "html", "sql", "yaml"] }],
              [
                rehypeSanitize,
                {
                  ...defaultSchema,
        ...
        <div className="mt-2 flex items-center justify-end gap-2">
          {!!sources?.length && (
            <button
              onClick={() => setOpen((s) => !s)}
              className="text-[11px] text-neutral-400 hover:text-neutral-100 transition"
            >
              {open ? "− sources" : `+ sources (${sources.length})`}
            </button>
          )}
          <button
            onClick={onCopyMessage}
            className={`text-[11px] transition ${
              messageCopied ? "text-green-400" : "text-neutral-400 hover:text-neutral-100"
            }`}
        ...
        {open && !!sources?.length && <SourcesList sources={sources} />}
      </div>
    </div>
  );
}
...
function SourcesList({ sources }: { sources: { path: string; score: number }[] }) {
  const base = process.env.NEXT_PUBLIC_VAULT_BASE_URL?.replace(/\/+$/, ""); // e.g. https://github.com/user/repo/blob/main
  return (
    <details open className="mt-2 text-xs text-neutral-300 border-t border-white/10 pt-2">
...
function encodePath(path: string) {
  // Encode each path segment so spaces & special chars survive in HTTP URLs.
...
const markdownComponents: import("react-markdown").Components = {
  ...
  // Prefer remark-slug ids if present (fall back to computed)
    ...
    return (
      <TagEl {...props} id={anchorId} className="group scroll-mt-20">
        ...
        {anchorId && (
          <a
            href={`#${anchorId}`}
            className="opacity-0 group-hover:opacity-100 text-xs ml-2 text-neutral-400 hover:text-neutral-200 transition-opacity no-underline"
...
// Apply a transform only to non-code segments split by fences
function normalizeOutsideFences(input: string, transform: (segment: string) => string) {
  const parts = input.split(/(```[\s\S]*?```)/g); // keep fences in result
